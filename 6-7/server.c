...
int main(int argc, char *argv[])
{
    ...
    {
        fprintf(stderr,"Usage:  %s <SERVER PORT> <WORK TIME> <LOG IP> <LOG PORT> <QEUE LENGTH>\n", argv[0]);
        exit(1);
    }
    ...
    for (processCt=0; processCt < 10; processCt++)
        ...
        if ((processID = fork()) < 0)
            ...
            ProcessMain(servSock, work_time, logsock, q_length);

    exit(0);  /* The children will carry on */
}

void ProcessMain(int servSock, int work_time, int logsock, int q_length)
{
    ...
    for (;;)  /* Run forever */
    {
        ...
        HandleTCPClient(clntSock, work_time, logsock, q_length);
    }
}
